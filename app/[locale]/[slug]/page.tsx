...
import { academyConfig, pageSlugs, type Locale, type PageSlug } from '@/lib/config';
import { faqs, formations, priceLabels, programme, seo, terms } from '@/lib/content';
import { FinalCta } from '../components/SiteChrome';

type Props = { params: Promise<{ locale: Locale; slug: PageSlug }> };

const allowed = pageSlugs.filter(Boolean);
...
function Hero({ locale, slug }: { locale: Locale; slug: PageSlug }) {
  const data = seo[locale][slug];
...
function Formations({ locale, slug }: { locale: Locale; slug: PageSlug }) {
...
function Programme({ locale, slug }: { locale: Locale; slug: PageSlug }) {
...
function Tarifs({ locale, slug }: { locale: Locale; slug: PageSlug }) {
...
function Faq({ locale, slug }: { locale: Locale; slug: PageSlug }) {
...
function Contact({ locale, slug }: { locale: Locale; slug: PageSlug }) {
...
function Terms({ locale, slug }: { locale: Locale; slug: PageSlug }) {
